...
def main(mpcap_file, json_file=None, frame_rate = 20, fit_window=1/6):
    ...
    if mpcap_file.endswith('.bvh'):
    ...
    elif mpcap_file.endswith('.npz'):
        print('npz file: ', mpcap_file)
        
        body_data  = np.load(mpcap_file)
        pos_data   = body_data['segment_tXYZ'].reshape(-1, 23, 3)
        frame_rate = int(body_data['frameRate'])

        xsense_to_world = np.array([[0, -1, 0], [1, 0, 0], [0, 0, 1]]) 
        root = pos_data[:, 0] @ xsense_to_world.T   # (R * pos.T).T
        ...
        if 'first' not in mpcap_file and 'second' not in mpcap_file and 'first_person' not in init_params.keys():
            update_property.append('first_person:mocap_sync')
            init_params['first_person'] = {'mocap_sync': peaks}
        ...
        pos_data, pos_data_bak, _ = load_csv_data(mpcap_file, mpcap_file)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-M", "--mpcap_file", type=str, default="D:\\Data\\20231222_s1_kenneth_fischer_act7_56uvqd\\Nymeria_v0.0_20231222_s1_kenneth_fischer_act7_56uvqd_body_motion\\body\\xdata.npz")

    parser.add_argument("-J", "--json_file", type=str, default="D:\\Data\\20231222_s1_kenneth_fischer_act7_56uvqd\dataset_params.json", 
